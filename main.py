...
#generación de cuerpos
    ...
    Cplanetas: int = config['simulacion']['cantidad.P']
    Clunas: int = config['simulacion']['cantidad.L']
    ...
    cuerpos: List[Any] = cuerpos_celestes.planetas+cuerpos_celestes.lunas

#cantidad de nucleos usados
    num_procesos: int = mp.cpu_count()-2
...
#crea una tupla por cada combinación de cuerpos, sin repetición 
        pares: List[Tuple[Any, Any]] = [(cuerpos[i], cuerpos[j]) for i in range(len(cuerpos)) for j in range(i+1, len(cuerpos))]
        
# Calcula la fuerza de los cuerpos con multiprocessing
        fuerza = pool.map(calcular_fuerza, pares)
        # print(fuerza)
# lista para fuerza de cuerpos
        fuerza_cuerpos_x: List[float] = [0.0]*len(cuerpos)
        fuerza_cuerpos_y: List[float] = [0.0]*len(cuerpos)
#lista de cuerpos a fusionar
        fusiones_pendientes: List[Tuple[Any, Any]] = []
#filtra los cuerpos a fusionar y lo agrega a funsiones pendientes y suma las fuerzas por componentes
        for a, (cuerpo1, cuerpo2) in enumerate(pares):
            i: int = cuerpos.index(cuerpo1)
            j: int = cuerpos.index(cuerpo2)
